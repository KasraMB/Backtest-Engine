# -*- coding: utf-8 -*-
"""
sweep7_new_strategies.py
------------------------
Sweeps IBS, TrendFollowing, and TurtleSoup strategies on NQ 2019-2024 IS data.
Results saved to sweep_results7/.
"""
...
from strategies.ibs_strategy import IBSStrategy
from strategies.trend_following_strategy import TrendFollowingStrategy
from strategies.turtle_soup_strategy import TurtleSoupStrategy
...
STRATEGIES = [
    {
        ...
        "grid": {
            "ibs_low_thresh":  [0.15, 0.20, 0.25, 0.30],
            "ibs_high_thresh": [0.70, 0.75, 0.80, 0.85],
    ...
    },
    {
        ...
        "eod_exit": dtime(23, 59),   # allow overnight holds
        "fixed": {
            "risk_per_trade":  0.01,
            "starting_equity": 100_000,
            "equity_mode":     "dynamic",
        ...
        "grid": {
            ...
            "sma_slow": [100, 200],
            "lookback": [50, 100, 200],
            "atr_mult": [3.0, 4.0, 6.0, 8.0],
        },
    },
    {
        "name": "TurtleSoup",
        "cls":  TurtleSoupStrategy,
        "eod_exit": dtime(16, 0),
        "fixed": {
            "risk_per_trade":  0.01,
            "starting_equity": 100_000,
        ...
        "grid": {
            "lookback_4h": [10, 15, 20, 30],
            "sl_atr_mult": [0.25, 0.5, 1.0],
...
def main() -> None:
    ...
    for strat_def in STRATEGIES:
        ...
        cfg = RunConfig(starting_capital=100_000, slippage_points=0.25,
                        commission_per_contract=4.50, eod_exit_time=eod_exit,
    ...
    with open(SUMM_PATH, "w") as f:
        f.write(f"Sweep7 IBS+TF+TS: {DATE_FROM} -> {DATE_TO}\n")
