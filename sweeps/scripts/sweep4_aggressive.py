# -*- coding: utf-8 -*-
"""
sweep4_aggressive.py
--------------------
Aggressive sweep: vary rr_ratio, sl_atr_multiplier, breakeven_r, atr_vol_filter,
and require_daily_momentum to push toward $40/day IS.

Fixed base: NY, BOS=True, momentum_only=True, disp=2.0, wick=0.15, atr=10

Grid:
  rr_ratio          : [0.75, 1.0, 1.25, 1.5]
  sl_atr_multiplier : [0.75, 1.0, 1.25, 1.5]
  breakeven_r       : [0.0, 0.5, 1.0]
  atr_vol_filter    : [0.0, 1.0]
  require_daily_mom : [False, True]
  Total: 4 × 4 × 3 × 2 × 2 = 192 combos
"""
...
FIXED_BASE = {
    ...
    "disp_min_atr_mult": 2.0, "wick_threshold": 0.15, "atr_period": 10,
...
OUT_DIR   = "sweep_results4"
PKL_DIR   = os.path.join(OUT_DIR, "pkl")
CSV_PATH  = os.path.join(OUT_DIR, "results.csv")
SUMM_PATH = os.path.join(OUT_DIR, "summary.txt")

CSV_COLS = ["combo_id","rr_ratio","sl_atr_multiplier","breakeven_r",
            "atr_vol_filter","require_daily_momentum",
...
def main():
    ...
    for idx, grid_params in enumerate(combos, start=1):
        ...
        print(f"[{idx:3d}/{total}] "
              f"rr={grid_params['rr_ratio']:.2f}  "
              f"sl={grid_params['sl_atr_multiplier']:.2f}  "
        ...
        if n_trades < MIN_TRADES:
            row = {"combo_id": cid, **{k: grid_params[k] for k in keys},
                   "n_trades": n_trades, **{k: None for k in CSV_COLS[7:-1]},
    ...
    print("\nTOP 20:\n" + top[["rr_ratio","sl_atr_multiplier","breakeven_r","atr_vol_filter",
                                "require_daily_momentum","n_trades","win_rate","avg_r",
                                "best_ev_per_day","best_account","best_pass_rate"]].to_string())
    with open(SUMM_PATH, "w") as f:
        f.write(f"Sweep4 (aggressive): {DATE_FROM} -> {DATE_TO}\n")
    ...
    for rank, (_, row) in enumerate(df_v.head(TOP_N_FULL).iterrows(), start=1):
        ...
        print(f"  [{rank:2d}] rr={gp['rr_ratio']:.2f} sl={gp['sl_atr_multiplier']:.2f} "
              f"be={gp['breakeven_r']:.2f} vol={gp['atr_vol_filter']:.2f} "
