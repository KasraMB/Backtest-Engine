...
sweep5_technical.py
-------------------
Sweeps all 9 technical indicator strategies on NQ 2019-2024 IS data.
Each strategy gets its own param grid. Results saved to sweep_results5/.
"""
...
from strategies.london_breakout_strategy import LondonBreakoutStrategy
...
STRATEGIES = [
    {
        "name": "MACD",
        "cls": MACDStrategy,
        "fixed": {**BASE_PARAMS, "atr_period": 14},
        "grid": {
            "ma_fast":           [10, 20],
            "ma_slow":           [50, 100, 200],
    ...
    {
        "name": "DualThrust",
        "cls": DualThrustStrategy,
        "fixed": {**BASE_PARAMS, "atr_period": 14, "k_lower": 0.5, "entry_window_minutes": 240},
        "grid": {
            "lookback_days":     [1, 3, 5],
            "k_upper":           [0.3, 0.5, 0.7],
            "rr_ratio":          [1.0, 1.5, 2.0],
    ...
    },
    {
        ...
        "fixed": {**BASE_PARAMS, "atr_period": 14},
        "grid": {
            "pre_session_start_hour": [4, 7],
            "entry_window_minutes":   [30, 60, 120],
            "rr_ratio":               [1.0, 1.5, 2.0],
            "sl_atr_multiplier":      [0.5, 1.0, 1.5],
        },
    },
    ...
    },
    {
        ...
        "cls": AwesomeOscillatorStrategy,
        "fixed": {**BASE_PARAMS, "atr_period": 14, "ao_fast": 5, "ao_slow": 34},
        "grid": {
            "signal_type":       ["zero_cross", "saucer"],
            "rr_ratio":          [1.0, 1.5, 2.0],
            "sl_atr_multiplier": [0.5, 1.0, 1.5],
        },
    },
    {
        "name": "ShootingStar",
        "cls": ShootingStarStrategy,
        "fixed": {**BASE_PARAMS, "atr_period": 14, "lower_wick_factor": 0.2, "upper_wick_factor": 2.0},
        "grid": {
            "body_size_atr_factor": [0.2, 0.4],
            "detect_hammer":        [True, False],
            "rr_ratio":             [1.0, 1.5, 2.0],
            "sl_atr_multiplier":    [0.5, 1.0, 1.5],
        },
    },
    {
        ...
        "grid": {
            "bb_period":         [10, 20],
            "bb_std":            [1.5, 2.0, 2.5],
            "signal_type":       ["mean_reversion", "breakout"],
            "rr_ratio":          [1.0, 1.5, 2.0],
            "sl_atr_multiplier": [0.5, 1.0, 1.5],
        },
    },
...
def main() -> None:
    ...
    print(f"Total combos across all strategies: {total}\n")
    ...
    for strat_def in STRATEGIES:
        ...
        for combo_vals in combos:
            ...
            print(f"n={n_trades:5d} WR={wr:.1%} avgR={avg_r:+.3f} "
                  f"ev/d={f'${ev:.2f}' if ev else 'N/A':>10} "
    ...
    with open(SUMM_PATH, "w") as f:
        f.write(f"Sweep5 technical: {DATE_FROM} -> {DATE_TO}\n")
        ...
        for sname in [s["name"] for s in STRATEGIES]:
            sub = df_v[df_v["strategy"] == sname]
            if len(sub) == 0:
                f.write(f"  {sname}: no valid results\n")
            else:
                best = sub.iloc[0]
                f.write(f"  {sname}: ev=${best['best_ev_per_day']:.2f}/day  "
                        f"n={int(best['n_trades'])}  WR={best['win_rate']:.1%}  "
                        f"avgR={best['avg_r']:+.3f}  {best['params_json']}\n")
