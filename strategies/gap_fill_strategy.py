...
class GapFillStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: MarketData, i: int) -> Optional[Order]:
        ...
        # Always track last close seen this session
        self._session_close = float(data.close_1m[i])

        # Day rollover: carry prior session close forward
        ...
        # Skip if the 9:30 bar itself already filled most of the gap (instantaneous fill)
        curr_close = float(data.close_1m[i])
        remaining  = abs(curr_close - self._prev_close)
        if remaining < gap_abs * 0.4:   # >60% filled within first bar — move is over
            return None
